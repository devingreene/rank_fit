...
from rank_fit.elo.ab_dict import ab_dict

__all__ = ['ab_fun']

def ab_fun(x):
    ''' Implements Elo-ranking function defined in [1] '''
    X = abs(x)
    for _e in reversed(ab_dict):
        if X >= _e[0]:
            if x >= 0: return 1 - _e[1]
            # else 
            return  _e[1]
    else:
        # This will never happen...
        raise Exception('Bad argument value: %d' % x)
